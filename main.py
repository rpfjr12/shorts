...
def cmd_apply_style(args):
    ...
    out_dir = Path(args.out) if args.out else (
        ROOT / "output" / "shorts" / date.today().isoformat()
    )
...
def cmd_run(args):
    print(
        ...
        f"format={args.format} count={args.count} palette={args.palette} duration={args.duration} ==="
    )

    scripts_dir = cmd_generate_scripts(
        argparse.Namespace(format=args.format, count=args.count, out=None)
    )
    images_dir = cmd_generate_images(
        argparse.Namespace(
    ...
    output_dir = cmd_apply_style(
        argparse.Namespace(
            clips_folder=final_clips_dir,
            palette=args.palette,
            out=None,
        )
    )
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Free automated YouTube Shorts pipeline"
    )
    ...
    p2 = sub.add_parser(
        "generate-images", help="Generate images for each script segment"
    )
    ...
    p2.add_argument(
        "--style",
        default=None,
        help="Style preset name from config/visual_style.json",
    )
    p2.add_argument("--palette", default="neon_blue_purple")
    p2.set_defaults(func=cmd_generate_images)

    p3 = sub.add_parser(
        "apply-motion", help="Apply zoom/pan motion effects to images"
    )
    p3.add_argument("--images-folder", required=True)
    p3.add_argument("--out", default=None)
    # Slower default for better readability
    p3.add_argument("--duration", type=float, default=2.5)
    p3.set_defaults(func=cmd_apply_motion)

    p4 = sub.add_parser(
        "add-text", help="Overlay script text onto motion clips"
    )
    ...
    p5 = sub.add_parser(
        "apply-style", help="Color-grade clips and export final Shorts"
    )
    ...
    # Slower default for full pipeline
    p6.add_argument("--duration", type=float, default=2.5)
